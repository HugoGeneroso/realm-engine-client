...
// are the same fixed PUBLIC values hardcoded in the client (InternalBridge.ts)
// and the C++ fallback (Handshake.cpp / IpcBridge.cpp), so prod matches the
// working dev configuration exactly. The HMAC handshake still runs but provides
// no protection (the key is public) — kept only to avoid touching the pipe
// protocol on both sides.
const handshakeKey = '47eb249907eb980c851fe3a7bdb56a244244bb7d465572b556e810df6827ecfb';
const pipeName = '\\\\.\\pipe\\lfg-dev-bridge';

// Write BuildSecrets.h so the C++ Release build compiles (it #includes this and
// #errors without it). It now contains only the fixed public constants above.
...
log('BuildSecrets.h written (fixed public constants)');
