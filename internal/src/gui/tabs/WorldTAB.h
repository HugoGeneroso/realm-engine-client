...
namespace WorldTAB {
    ...
    // True on realm/dungeon maps (not Nexus, Vault, or character-select scale).
    // Uses map name heuristics plus WorldManager +0xFC/+0x100 size (256 hub vs 2048 realm).
    bool IsPlayableRealmMap();

    // Lightweight entity-dict scan for proxy NEWTICK objectId — no tile rebuild.
