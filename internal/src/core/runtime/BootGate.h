...
namespace BootGate {
    ...
    // True when a local character is loaded (memory ptr, wire stats, or position).
    bool CharacterInWorld();
    // IL2CPP walks for lazy projectile/AoE classes — only after Ready + in-world.
    bool LazyOffsetLookupAllowed();
    // Re-run anchor audit without leaving Ready (after lazy offset heal/finalize).
