...
void WriteSnapshot(const char* dir) {
    ...
    const AutoAim::DiagView av = AutoAim::GetDiagView();
    if (len > 0 && len < static_cast<int>(sizeof(buf)) - 256)
        len += snprintf(buf + len, sizeof(buf) - len,
            "  \"autoAim\": { \"requested\": %s, \"hooks\": %s, \"hasTarget\": %s,\n"
            "    \"enemies\": %d, \"pods\": %d, \"aimX\": %.2f, \"aimY\": %.2f,\n"
            "    \"csa\": \"%llu/%llu\", \"swa\": \"%llu/%llu\" }\n",
            av.aimRequested ? "true" : "false",
            av.hooksInstalled ? "true" : "false",
            av.hasTarget ? "true" : "false",
            av.enemyCount, av.podCount, av.aimX, av.aimY,
            static_cast<unsigned long long>(av.csaCalls),
            static_cast<unsigned long long>(av.csaRedirect),
            static_cast<unsigned long long>(av.swaCalls),
            static_cast<unsigned long long>(av.swaRedirect));
